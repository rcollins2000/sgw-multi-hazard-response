...
|---|---|---|---|
| **Hazard-conditional asset risk scoring** | Gradient-boosted regressor (LightGBM v2) + Random Forest baseline. **Real probability calibration is Phase 2** — the current implementation predicts a raw 0..1 stress score against a synthetic label (fictional utility has no real failure history); production replaces the label with historical incident joins, at which point a calibrated classifier becomes appropriate. | Per-asset stress score with contributing factors, given (hazard type, severity, region) | Rules can't blend continuous forecast features with categorical asset attributes and rare-event history at scale |
