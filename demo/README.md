...
The UI landed 2026-07-17 as the **Storm Cockpit** — see [docs/09_design_cockpit_v2.md](../docs/09_design_cockpit_v2.md) for the design rationale. The landing screen surfaces "what does AI recommend I do right now?" as its single job. Point at, in order:
...
2. **Platform status strip (thin row under the command bar)** — models ready pill + `risk model lgbm-cal-v1` + `ROC-AUC 0.804` + `Brier 0.175` + `graph mod. 0.901` + `copilot gpt-oss:120b`. All values are live from `/api/status`.
...
6. **CopilotPullQuote** — amber-left-border card, "Copilot recommends" eyebrow. Renders the LLM's recommended action + evidence chips + "gpt-oss:120b · structured · advisory · schema-validated" footer. This is the visual grammar that keeps the LLM as *copilot, never producer*.
...
The **Scenarios** tab surfaces the agent — see [docs/11_scenario_agent.md](../docs/11_scenario_agent.md) for the full pipeline. Demo the three flows in this order:
...
5. **Free-text directive** — for the second run, paste something like *"What if a Cat 4 hurricane made landfall at Savannah in 21 days?"* into the directive textarea and Run. The LLM parses to a fresh `ScenarioSpec` (real endpoint calls gpt-oss:120b; falls back to a neutral synthesised spec if parsing fails). Same UI, LLM-parsed spec instead of preset.
...
**Real:**
- Ollama Cloud `gpt-oss:120b` responses (live LLM calls with structured JSON output validated via Pydantic)
- **NWS active alerts — polled live every 60 s** for SC/GA/NC via `api.weather.gov`. Upserted by `alert_id` into `weather_alerts`. Rows expired > 24h are swept. See [docs/10_live_data_polling.md](../docs/10_live_data_polling.md).
...
- Models: risk (LightGBM v2 regressor + RF baseline — isotonic calibration deferred to Phase 2 pending real failure labels), forecasting (Prophet with M2 tidal seasonality — 80% nominal band, ~58% empirical coverage disclosed on Governance), anomaly (Prophet-residual + rolling-median outlier ranking), optimisation (OR-Tools VRP + Guided Local Search), graph (networkx BFS + Louvain), fairness auditing, operator-preference alignment layer (supervised preference learning, bounded corrective nudge)
- LLM: Ollama Cloud, `gpt-oss:120b`, structured output via schema-in-prompt-AND-format pattern, Pydantic validation + retry
